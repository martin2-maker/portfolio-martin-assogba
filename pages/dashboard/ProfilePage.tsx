...
const ProfilePage = () => {
    ...
    const handleConfirmReset = async () => {
        ...
        try {
            ...
            if (failedDeletes.length > 0) {
                console.error("Échec de la réinitialisation des données pour certaines tables:", failedDeletes);
                // @ts-ignore
                const errorMessages = failedDeletes.map((f, i) => `${tablesToReset[i]}: ${f.reason?.message || 'Unknown error'}`).join('; ');
                showToast(`Certaines données n'ont pas pu être réinitialisées. Erreur: ${errorMessages}`, true);
            } else {
            ...
            console.error("Le processus de réinitialisation des données a échoué:", err);
            showToast("Une erreur inattendue est survenue lors de la réinitialisation.", true);
        } finally {
            setLoading(prev => ({ ...prev, reset: false }));
        }
    };
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            ...
            const failedDeletes = results.filter(r => r.status === 'rejected');
            if (failedDeletes.length > 0) {
                console.error("Échec de la suppression des données pour certaines tables:", failedDeletes);
            }
    
            const { error: updateError } = await supabase.auth.updateUser({
                data: { deactivated: true, full_name: "Compte Supprimé", avatar_url: null } 
            });
            ...
            console.error("Le processus de suppression a échoué:", err);
            showToast(`Une erreur est survenue lors de la suppression du compte: ${err.message}`, true);
        } finally {
            setLoading(prev => ({ ...prev, delete: false }));
        }
    };
    ...
    return (
        <div className="max-w-4xl mx-auto text-white space-y-12">
            ...
            <ConfirmationModal
                ...
                message="Cette action supprimera toutes vos données (projets, tâches, notes, etc.) de manière permanente. Votre compte de connexion sera désactivé et vous serez déconnecté. Êtes-vous certain de vouloir continuer ? Cette action est irréversible."
            />
